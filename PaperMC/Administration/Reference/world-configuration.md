...
- **默认值**: `false`
- **描述**: 是否要隐藏服务器发送给客户端耐久度相关信息。可能会影响需要依赖于获取耐久度的材质包。

##### hide-itemmeta

- **默认值**: `false`
- **描述**: 发送给玩家客户端的数据中是否要隐藏不必要的物品信息（例如附魔、物品在潜影盒/背包中 等等），这部分数据可能会为作弊客户端带来有利条件。可能会影响需要依赖于获取附魔、Lore或物品名称的材质包。
...
- **默认值**: `-1`
- **描述**: 配置世界在间隔多少Tick后应当保存一次。此值优先度将高于 `bukkit.yml` 文件中 `ticks-per.autosave`。若此值设为 `-1`，则默认使用 `bukkit.yml` 文件中的`ticks-per.autosave`。
...
- **描述**: 限制每个区块中指定实体可被保存/加载的数量。 设定此值为 `-1` 将会禁用对特定实体的限制。可以添加除了已经填写的实体类型以外的任何实体。

### fixed-chunk-inhabited-time

- **默认值**: `-1`
- **描述**: 如果设定为`0`或更高，则将区块的保持加载时间设定为一个定值。这里的“定值” 是指静态或永不会变，是**不是**指修复一个Bug。一旦玩家在此区块活跃，计时器就会开始计数。

### max-auto-save-chunks-per-tick

- **默认值**: `24`
- **描述**: 单个Tick中自动保存最大可保存区块的数量。

### prevent-moving-into-unloaded-chunks

- **默认值**: `false`
- **描述**: 服务器是否要允许玩家移动到尚未加载的区块之中。
...
- **默认值**: `false`
- **描述**: 当实体堆叠数量超过游戏规则 `maxEntityCramming` 设定值时是否要对玩家造成伤害。
...
- **默认值**: `true`
- **描述**: 禁用后，盔甲架将停止计算Tick。此项在有大量盔甲架时可以提高性能。
...
- **默认值**: `false`
- **描述**: 禁用苦力怕的随机AreaEffectCloud效果。

#### disable-player-crits

- **默认值**: `false`
- **描述**: 在PvP中是否要禁用暴击，使每一次挥剑的伤害相同。
...
- **默认值**: `false`
- **描述**: 服务器是否应该完全阻止由爆炸操作的击退。
- 
### disable-ice-and-snow

- **默认值**: `false`
- **描述**: 是否不再形成冰与雪。同时这也会使炼药锅不再会被雨/雪填满。

### disable-teleportation-suffocation-check

- **默认值**: `false`
- **描述**: 是否要在尝试传送玩家之前禁用窒息检查。
- **说明**: 虽然这会帮助玩家传送出去时不被墙所阻挡，但是在特定位置玩家当登出服务器时也会使其穿过方块。
...
- **默认值**: `true`
- **描述**: 服务器是否要启用（并Tick）霜冰方块。译者注：根据Minecraft Wiki得知，霜冰应该是冰霜行者附魔创造出的方块。
...
- **默认值**: `16`
- **描述**: 服务器尝试生成新传送门可以偏移的最大距离。

### portal-search-radius

- **默认值**: `128`
- **描述**: 服务器寻找已有传送门的范围。如果找不到就新生成一个。译者注：可以适当把此值调大一点，因为译者在玩游戏时经常在地下生成一个新的传送门。

### portal-search-vanilla-dimension-scaling

- **默认值**: `true`
- **描述**: 是否对原版维度应用 `portal-search-radius`。
...
- **默认值**: `true`
- **描述**: 从村民和箱子中得到的藏宝图是否能够指向一份宝藏。（原文是feature，我认为可能是拼写错误，应该为treasure）
...
- **默认值**: `default`
- **描述**: 为未被探索过的地方覆盖配置战利品表检查。`default` 允许战利品表将单独确定搜索位置时是否准许已探索过的位置。原版战利品表都会自动跳过已探索过的位置，因此将此项设置为`false` 会覆盖并强制搜索已探索过的位置。
...
- **默认值**: `false`
- **描述**: 为全部未指定种子的部分启用自动填充种子。在受限环境中启用此项是自动填充种子的好办法。

### `<feature-namespace>`

- **描述**: 在特定情形下设定种子种群。如果设定为`-1`则保持原版种子种群不变，也不会被自动填充取代。
...
- **默认值**: `true`
- **描述**: 防止在卸载区块时同时存储投出的末影珍珠。

### falling-block-height-nerf

- **默认值**: `0`
- **描述**: 服务器应该移除下落方块的高度。设定小于 `1` 的值将禁用此项。

### fix-curing-zombie-villager-discount-exploit

- **默认值**: true
- **描述**: 修复一个反复治愈僵尸村民以得到村民折扣的[恶意利用的机制](https://bugs.mojang.com/browse/MC-181190)。

### fix-items-merging-through-walls

- **默认值**: `false`
- **描述**: 是否应该防止物品堆叠可穿过墙。启用此项会使性能下降。只有当 `spigot.yml`中的`merge-radius.item`很大才会隔墙堆叠物品。

### prevent-tnt-from-moving-in-water

- **默认值**: `false`
- **描述**: 服务器是否需要禁止已激活的TNT在水中游动。

### split-overstacked-loot

- **默认值**: `true`
- **描述**: 当设定为 `false` 时，战利品表不会将超出最大堆叠限制的物品尝试进行拆分为较少的堆叠数量。这会阻止超过最大堆叠数目导致在生成模式下的潜影盒中物品丢失或使区块崩溃而出现错误（因为玩家一直在尝试发送过大的包，最终导致被踢出服务器）
...
- **默认值**: `true`
- **描述**: 当漏洞已满时是否要暂时停止加入新物品来替代不断尝试往里塞东西。
...
- **默认值**: `false`
- **描述**: 服务器是否会自动填充战利品容器。启用此项会为部分玩家不断跑图并不断生成新区块以寻找好东西的长期存档的服务器非常有用。
...
- **默认值**: `128`
- **描述**: 每个地图最大标记数量。如果过多可能会导致客户端出现延迟（FPS下降）。
...
- **默认值**: `false`
- **描述**: 当玩家处于疾跑时受到攻击是否要停止疾跑。

### light-queue-size

- **默认值**: `20`
- **描述**: 设定每个世界对于非主线程的光更新队列的大小。原版设定此值为 `5`，但是实际使用此值会造成许多问题，特别是WorldEdit等插件。
