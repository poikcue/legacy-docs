...
- **默认值**: `-1`
- **描述**: 服务器使用多少线程用于保存世界和加载区块。默认值（-1）将使用服务器线程数量的一半进行区块加载，除非有别的内容规定了此项，并且默认使用最大4个区块用于保存和加载区块。可以使用 `-Dpaper.maxChunkThreads=[数字]` 启动参数进行调整。
...
- **默认**: `true`
- **描述**: 是否要根据客户端设定的可视距离作为服务器实际发送给客户端的可视距离。仅有此项会设定服务器发送给客户端的区块距离的半径，但是不会影响服务器实际加载和Tick。
...
- **默认值**: `-1.0`
- **描述**: 服务器每秒加载区块的最大数值。优先级要大于 `player-max-chunk-load-rate`。
...
- **默认值**: `2`
- **描述**: 每次排队发送区块的最大数值。较低的值会缓解服务器的网络瓶颈，比如使用反矿物透视和压缩。但是，此值可能不会如预期的优化玩家在使用差些的网络环境时的游戏体验。

### min-load-radius

- **默认值**: `2`
- **描述**: 在区块加载时，玩家周围未造成阻塞的范围（值为半径）。此值影响的实际范围是本值+1，并且此项会控制客户端可加载的区块数量。
...
- **默认值**: `true`
- **描述**: 服务器是否应该开启玩家之间的碰撞箱。此选项在与安装某些需要玩家与计分板交互的插件时，可能不起作用。所以如果在调整使用此配置时出现问题，请尝试先禁用这些插件。

### send-full-pos-for-hard-colliding-entities

- **默认值**: `true`
- **描述**: 当与船只或矿车碰撞时，客户端/服务器经常会导致两者在处理时不一致，并且会使玩家的行为出错。此选项尝试在其碰撞时发送精确的位置来缓解此问题。开启此选项会消耗更多的带宽，但是在大多数情况下值得开启此选项。
...
- **默认值**: `true`
- **描述**: 当自动补全找不到其它参数可用时，将尝试返回Tab列表中的玩家名称。

### time-command-affects-all-worlds

- **默认值**: `false`
- **描述**: 当输入 `/time` 命令时，应该在所有世界生效还是只在命令发送者所在的世界生效。
...
- **默认值**: `false`
- **描述** 是否要在书中解析选择器。当启用后，创造模式的玩家可以使用一种方式导致服务器崩溃。
...
- **默认值**:
  ...
  > 中文参考翻译：`<red>我对此很抱歉，但是你的权限不足以使用此命令。如果你认为这是一个错误，请与服务器管理员联系。`
- **描述**: 当玩家权限不足以运行某命令或进行某操作时提示的文本。插件可以为其新增的命令使用不同的提示文本。

### use-display-name-in-quit-messages

- **默认值**: `false`
- **描述**: 当玩家退出服务器时应该用显示名称（可被插件修改，即Display name）还是真实的名称。
...
- **默认值**: 连接阻塞（即点击加入服务器按钮后极快速的断开连接并重新请求加入）时发送给玩家的消息。 可以在 `bukkit.yml` 配置。
...
- **默认值**: `<lang:multiplayer.disconnect.flying>`
- **描述**: 当玩家被检测到骑着或驾驶某物时飞行作弊时踢出服务器显示的消息。
...
- **默认值**: `true`
- **描述**: 是否修复服务器与客户端实体同步位置不精准的Bug。修复 [MC-4](https://bugs.mojang.com/browse/MC-4) 的Bug。

### lag-compensate-block-breaking

- **默认值**: `true`
- **描述**: 服务器是否需要使用时间或TPS判断破坏方块时间。客户端以服务器的TPS始终为20为标准，导致在服务器出现延迟时，破坏方块出现两者不一致的情况。此设置即避免出现此情况。

### load-permissions-yml-before-plugins

- **默认值**: `true`
- **描述**: 是否要在在加载插件之前优先载入permissions.yml文件，允许在启动时立刻读取其数据。

### max-joins-per-tick

- **默认值**: `3`
- **描述**: 设定每个Tick加入服务器的玩家的最大数量。如果玩家加入时超过了此限制，则会往后推迟加入时间。与 `Bukkit.yml` 配置文件中的 `connection-throttling` 没有任何关系。
...
- **默认值**: `false`
- **描述**: 是否要为幸运应用新的 [计算方式](https://gist.github.com/aikar/40281f6c73ec9b6fef7588e6461e1ba9)。同时，未定义的部分游戏内容也将享受幸运增益。这对钓鱼计算公式有重大变化。

### use-dimension-type-for-custom-spawners

- **默认值**: `false`
- **描述**: 幻翼，流浪商人是否应该在自定义主世界中生成。默认为false以与原版相契合。
...
- **默认值**: `KICK`
- **描述**:  一旦触及限制时需要采取的处罚措施。当填入 `DROP` 时将忽略超过限制的包，`KICK`将在触及限制时踢出该名玩家。
...
- **默认值**: `-1`
- **描述**: 每Tick最多保存多少名玩家的数据。设定为默认值 `-1` 将会根据 `player-auto-save.rate` 设定一个推荐数值，不是`10`就是`20`。
...
- **默认值**: `false`
- **描述**: 服务器是否需要传播 [代理协议](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) 消息文本。 这和Velocity或是BungeeCord完全没有关系，仅当你使用HAProxy或类似软件时才应当启用。
...
- **默认值**: `false`
- **描述**: 许多计分板插件遗留了大量的空的计分板Teams，大大减缓了登录速度。是否要自动清理这些无用的计分板Teams。

### track-plugin-scoreboards

- **默认值**: `false`
- **描述**: 服务器是否跟踪计分板插件时只跟踪dummy类型的计分项。这是一个突破性的变化，但是默认值是合理的，因为当你使用某些计分板插件时同时启用此项会减缓性能。
...
- **描述**: 指定一个 [Timings Viewer](https://github.com/aikar/timings) 服务器，并将以后的Timings报告发送到该服务器上。
...
- **默认值**: `false`
- **描述**: 是否允许服务器中出现（或玩家故意造成）无头活塞，通常用它来永久破坏正常无法破坏的方块（比如下界破坏基岩天花板）。
